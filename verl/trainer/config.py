...
class TrainerConfig:
    ...
    val_generations_save_freq: int = 5
    """write `val_generations_step_*.jsonl` every N global steps; `val_generations_latest.jsonl` is always updated"""
    val_generations_print_num: int = 2
    """print N validation generations to console when `val_generations_step_*.jsonl` is written"""
    val_generations_print_max_chars: int = 800
    """max chars per printed field (prompt/output) for console monitoring"""
    save_freq: int = -1
    """save frequency, -1 means no saving"""
    save_limit: int = -1
    """max number of checkpoints to save, -1 means no limit"""
    save_keep_steps: Tuple[int, ...] = ()
    """checkpoint steps to keep regardless of save_limit"""
