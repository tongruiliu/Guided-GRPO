...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self) -> None:
        ...
        remove_obsolete_ckpt(
            ...
            keep_steps=self.config.trainer.save_keep_steps,
        )
    ...
    def _maybe_log_val_generations(
        ...
        for idx, (inp, out, label, score) in enumerate(samples, start=1):
            print("\n" + "=" * 20 + f" Validation Sample {idx} (step {self.global_step}) " + "=" * 20)
            print(f"[Prompt]\n{inp.strip()}\n")
    ...
    def _write_val_generations_to_file(
        self, inputs: list[str], outputs: list[str], labels: list[str], scores: list[float]
    ) -> None:
        if len(inputs) == 0 or not self.config.trainer.save_checkpoint_path:
            return

        save_dir = Path(self.config.trainer.save_checkpoint_path)
        save_dir.mkdir(parents=True, exist_ok=True)
        latest_file = save_dir / "val_generations_latest.jsonl"

        records = []
        for inp, out, label, score in zip(inputs, outputs, labels, scores):
            records.append(
                {
        ...
        save_freq = int(getattr(self.config.trainer, "val_generations_save_freq", 5))
        write_step_file = save_freq > 0 and (self.global_step % save_freq == 0)
        paths = [latest_file]
        if write_step_file:
            paths.insert(0, save_dir / f"val_generations_step_{self.global_step}.jsonl")
        ...
        if write_step_file:
            self._print_val_generations_to_console(
                inputs=inputs,
                outputs=outputs,
                labels=labels,
                scores=scores,
                num_samples=int(getattr(self.config.trainer, "val_generations_print_num", 5)),
                max_chars=int(getattr(self.config.trainer, "val_generations_print_max_chars", 800)),
            )

    def _print_val_generations_to_console(
        self,
        inputs: list[str],
        outputs: list[str],
        labels: list[str],
        ...
        print("\n" + "=" * 20 + f" Validation Monitor (step {self.global_step}) " + "=" * 20)
        for idx, (inp, out, label, score) in enumerate(samples, start=1):
            print(f"\n[{idx}] score={float(score):.4f} label={label}")
            print(f"[Prompt]\n{_clip(inp)}")
            print(f"[Output]\n{_clip(out)}")
        print("\n" + "=" * 60 + "\n")
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_gen_batch = test_batch.pop(
                ...
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "messages"],
            )
        ...
        self._write_val_generations_to_file(sample_inputs, sample_outputs, sample_labels, sample_scores)
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            if "ground_truth" in new_batch.non_tensor_batch:
                gen_batch.non_tensor_batch["ground_truth"] = new_batch.non_tensor_batch["ground_truth"]
