...
_MODAL_MISMATCH_WARNED = {"image": False, "video": False}


def _align_modal_features(features: torch.Tensor, required_tokens: int, modal_name: str) -> torch.Tensor:
    """Ensure vision token count matches placeholder tokens; truncate/pad if mismatch to avoid runtime errors."""
    ...
    if required_tokens <= 0:
        return features
    if current > required_tokens:
        return features[:required_tokens]
    pad = torch.zeros(
        (required_tokens - current, *features.shape[1:]),
...
def _log_modal_mismatch(modal: str, tokens: int, feats: int) -> None:
    if not _MODAL_MISMATCH_WARNED.get(modal, False):
        print(f"[warn] {modal} tokens ({tokens}) != {modal} features ({feats}); auto align to tokens.")
        _MODAL_MISMATCH_WARNED[modal] = True
...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        if n_image_tokens != n_image_features:
            _log_modal_mismatch("image", n_image_tokens, n_image_features)
            image_embeds = _align_modal_features(image_embeds, n_image_tokens, "image")
            if deepstack_image_embeds is not None:
                deepstack_image_embeds = [_align_modal_features(emb, n_image_tokens, "image") for emb in deepstack_image_embeds]
    ...
    if pixel_values_videos is not None:
        ...
        if n_video_tokens != n_video_features:
            _log_modal_mismatch("video", n_video_tokens, n_video_features)
            video_embeds = _align_modal_features(video_embeds, n_video_tokens, "video")
            if deepstack_video_embeds is not None:
                deepstack_video_embeds = [_align_modal_features(emb, n_video_tokens, "video") for emb in deepstack_video_embeds]
...
def qwen3_vl_base_forward(
    ...
    **kwargs,
    ...
    if isinstance(position_ids, torch.Tensor):
        # Normalize position_ids shape for compatibility:
        # - some pipelines pass (batch, 4, seq_len); permute to (4, batch, seq_len)
        # - some pass (batch, seq_len); expand to 4 heads
        if position_ids.ndim == 3 and position_ids.size(0) != 4 and position_ids.size(1) == 4:
