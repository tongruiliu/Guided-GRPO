...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self.image_key in example:
            ...
            if num_images > 0:
                marker = "<image>"
                marker_count = prompt_str.count(marker)
                if marker_count == 0:
                    prompt_str = f"{marker}\n{prompt_str}"
                elif marker_count > num_images:
                    parts = prompt_str.split(marker)
                    prompt_str = marker.join(parts[: num_images + 1]) + "".join(parts[num_images + 1 :])
                elif marker_count < num_images:
                    prompt_str = (marker + "\n") * (num_images - marker_count) + prompt_str
